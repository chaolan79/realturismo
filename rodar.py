...
def obter_dados_manutencoes(filtro_status=None, session_instance=None):
    if not session_instance:
        return pd.DataFrame()
    try:
        # Removido o filtro tem_vencimento == True para capturar todas as manutenções
        ...
        for m in manutencoes:
            ...
            dados_manutencoes.append({
                "ID": m.id, "Tipo de Registro": "Manutenção", "Veículo": veiculo_nome, "Categoria": m.categoria, 
                "Responsável": m.responsavel, "Oficina": m.oficina, "Tipo": adicionar_emoji_tipo(m.tipo), 
                "KM Aviso": m.km_aviso, "KM Aviso (km)": f"{formatar_valor_ptbr(m.km_aviso)} km", 
                "Data Manutenção": m.data_manutencao, "Hodômetro": m.hodometro_manutencao, 
                "Hodômetro (km)": f"{formatar_valor_ptbr(m.hodometro_manutencao)} km", 
    ...
    try:
        ...
        for a in acessorios:
            ...
            dados_acessorios.append({
                ...
                "Valor (R$)": 0.0, "Valor Formatado (R$)": "N/A"  # Acessórios não têm valor
            })
